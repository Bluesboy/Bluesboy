  ...
  [
    ...
    #_join_with_dividers(valid, lang => block(
      breakable: false,
      {
        ...
        if value != none {
          // LOCAL PATCH (Bluesboy/cv): `fluency` is unused by upstream
          // once a numeric `rating` wins in `_resolve_rating`; here it
          // rides along as the printed level beside the dots.
          rating(lang.language, value, caption: lang.at("fluency", default: none))
        } else {
