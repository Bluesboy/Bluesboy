...
#let _network_icons = (
  ...
  // LOCAL PATCH (Bluesboy/cv): upstream has no Telegram entry, so this
  // CV used to route Telegram through the generic `link` mark and then
  // swap the glyph with a document-wide `#show "\u{f0c1}"` rule in
  // resume.typ — which rewrote that codepoint anywhere it appeared.
  telegram: "telegram",
  twitter: "twitter",
