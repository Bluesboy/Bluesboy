...
#import "state.typ": _body_size_state, _accent_state, _max_rating_state, _empty_dot_colour, _body_colour
...
// LOCAL PATCH (Bluesboy/cv): `caption`. Upstream renders either dots
// (numeric `rating`) or a text fluency (`_label_only`), never both. Dots
// alone carry no text for screen readers and lose their fill when a
// browser prints without background graphics, so this CV shows the CEFR
// level beside the dots rather than instead of them.
#let rating(label, value, caption: none) = context {
  ...
  // LOCAL PATCH (Bluesboy/cv): smaller dots. Upstream's 0.45/0.4 make a
  // seven-dot row ~87pt wide, which together with the language name and
  // the level caption overflows this CV's 152pt sidebar and wraps the
  // last dot onto its own line. 0.33/0.28 brings the row to ~63pt.
  let dot-radius = 0.33 * body-size
  let dot-baseline = -0.2 * body-size
  let dot-spacing = 0.28 * body-size

  text(label)
  h(1fr)
  if type(caption) == str and caption.len() > 0 {
    text(0.85 * body-size, fill: _body_colour, caption)
    h(0.6 * body-size)
  }
