...
#import "../sections/about.typ": _about
...
#let _sections = (
  ...
  // LOCAL PATCH (Bluesboy/cv): column "full" renders below the grid at
  // the page's full width — see `fullWidthSections`.
  about: (
    column: "full",
    render: (cv, labels, prefs) => _about(cv.at("about", default: ()), labels),
  ),
)
...
#let _default_preferences = (
  ...
  // LOCAL PATCH (Bluesboy/cv): sections rendered after the two-column
  // grid, spanning the full text width. A grid keeps its column widths
  // on every page, so whatever follows a long left column stays narrow
  // even once the right column has run out — these escape that.
  fullWidthSections: _keys_for_column("full"),
