...
// LOCAL PATCH (Bluesboy/cv): upstream draws a quiet dashed rule in
// lightened body colour. This CV uses the sub-heading as the visible
// grouping for the skill areas in the right column, so the rule is
// solid and both rule and label take the accent colour. Previously this
// was a `#show grid:` rule in resume.typ that matched
// `columns == (1.3em, auto, 1fr)` and walked `it.children.at(1).body.child`
// — it broke silently whenever this function changed.
#let _labelled_divider(label) = context {
  let body-size = _body_size_state.get()
  let accent = _accent_state.get()
  let stroke = (paint: accent, thickness: 0.7pt)
  v(0.3 * body-size)
  // Equal rules on both sides centre the label; upstream's leading 1em
  // pins it left. Smaller than body text so a long Russian group name
  // ("Скриптинг и автоматизация") stays on one line in a narrow column.
  pad(left: 0.6 * body-size, grid(
    columns: (1fr, auto, 1fr),
    column-gutter: 0.4 * body-size,
    align: horizon,
