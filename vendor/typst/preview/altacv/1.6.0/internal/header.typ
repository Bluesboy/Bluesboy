...
#let _header(
  ...
  context {
    ...
    let header-text = align(text-align, {
      ...
      // LOCAL PATCH (Bluesboy/cv): this CV packs role, work format and
      // salary into `basics.label`, so it needs to sit on one line at
      // A4 width. Upstream's 1.2x wraps it, and so does 1.05x once the
      // Russian role title is spelled out. Measured with pdftotext -bbox:
      // the line has room up to ~495pt, 0.98x lands at ~485pt. Previously
      // this was a `#show text.where(size: 12.6pt)` rule in resume.typ —
      // 12.6pt was just `1.2 * bodySize` precomputed, so it stopped
      // matching the moment `preferences.bodySize` changed.
      if "label" in basics and basics.label != none {
        block(
          spacing: 0pt,
          below: 0.8 * body-size,
          text(0.98 * body-size, fill: _emphasis_colour, weight: "bold", basics.label),
        )
      }
